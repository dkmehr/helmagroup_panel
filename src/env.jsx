const env = {
  ...
  siteApi: "https://admin.helmagroup.com/api",
  // siteApi: "https://demofahaadmin.dkmehr.com/api",

  // siteApiUrl:'http://localhost:6026',
  siteApiUrl: "https://admin.helmagroup.com",
...
export function notNull(array, item) {
  ...
  for (var i = 0; i < array.length; i++) {
    if (array[i][item]) notNullArray.push(array[i]);
  }
  return notNullArray;
}
...
export const payValue = (priceSet, payValue, count, off) => {
  ...
  if (off) {
    ...
    if (offInt > 100) rawPriceCount -= parseInt(off);
    else rawPriceCount = (rawPriceCount * (100 - parseInt(off))) / 100;
  }
...
export const parseDesc = (desc) => {
  ...
  if (!vs) vs = findElement(desc, "ویسکوزیته", 1);
  var suitable = findElement(desc, "مناسب برای", 3);
  if (!suitable) suitable = findElement(desc, "مناسب برای", 1);
...
const findElement = (desc, field, index) => {
  ...
  var pureValue = value;
  if (value && value.includes(":")) pureValue = value.replace(":", "");
  if (pureValue && pureValue[0] === " ") pureValue = pureValue.replace(" ", "");
  return pureValue;
};
...
export const findFPage = (user) => {
  ...
  if (userData) {
    if (userData.profileClass === "670221bc770a28e46a40da9a") return "sale";
